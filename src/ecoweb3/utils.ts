export function lookupOpcode(op: any, full: any) {
  //@ts-ignore
  let code = codes[op] ? codes[op] : ['INVALID', 0];
  let opcode = code[0];
  ...
  return { name: opcode, fee: code[1], in: code[2], out: code[3], dynamic: code[4] } as OPCODE
}
